package com.example.kalyani.newsapp;
    ...
    private static List<News> extractNews(String jsonResponse, Context context) {
        ...
        try {
            ...
            if(jsonObject.has(KEY_RESPONSE_OBJECT)){
                ...
                if(responseJSONObject.has(KEY_RESULT)) {
                    JSONArray resultsJSONArray = responseJSONObject.getJSONArray(KEY_RESULT);
                    for (int i = 0; i < resultsJSONArray.length(); i++) {
                        ...
                        if(currentJSONObject.has(KEY_TAGS)) {
                            JSONArray currentTagsArray = currentJSONObject.getJSONArray(KEY_TAGS);
                            if (currentTagsArray == null || currentTagsArray.length() == 0) {
                                ...
                                for (int j = 0; j < currentTagsArray.length(); j++) {
                                    ...
                                    authors.add(currentObjectInTags.getString(KEY_WEB_TITLE));
                                }
                            }
                        }else{
                            authors = null;
